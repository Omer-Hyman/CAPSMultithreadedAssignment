...
ReceivedSocketData TCPServer::accept()
{
	...
	returnedSocketData.ClientSocket = ClientSocket;
	return returnedSocketData;
}

void TCPServer::receiveData(ReceivedSocketData &clientSocket, bool blocking) {
	int numberOfBytesReceived;
	char buffer[DEFAULT_BUFFER_LENGTH];
	int bufferLength = DEFAULT_BUFFER_LENGTH - 1;
	struct pollfd pollDescriptor;	
	
	pollDescriptor.fd = clientSocket.ClientSocket;
	// TODO: make sure events and revents(return events) are right
	pollDescriptor.events = POLLIN; // normal and priority data may be read without blocking. Is priority data needed?
	pollDescriptor.revents = 0;

	int socketStatus; // status of listening socket

	clientSocket.request = "";
	clientSocket.reply = "";

	if (!blocking) // The WSAPoll function determines status of one or more sockets.
	{
		socketStatus = WSAPoll(&pollDescriptor, 1, 100); // check for events
		//cout << "\nreturnValue: " << socketStatus;
	}

	if (blocking || (!blocking && socketStatus > 0)) // if the clientSocket is busy. Setup buffer?
	{

		numberOfBytesReceived = recv(clientSocket.ClientSocket, buffer, bufferLength, 0);
		if (numberOfBytesReceived > 0) {
			buffer[numberOfBytesReceived] = '\0'; // \0 = null character
			clientSocket.request = string(buffer);
		} 
